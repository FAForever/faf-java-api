...
public class UserService {
  ...
  CallbackResult performPasswordResetViaSteam(HttpServletRequest request, String token) {
    ...
    Map<String, String> attributes = fafTokenService.resolveToken(FafTokenType.PASSWORD_RESET_STEAM, token);
    ...
    if (errors.isEmpty()) {
      userPasswordResetDoneViaSteamCounter.increment();
    } else {
  ...
  public String buildSteamPasswordResetUrl(String callbackUrl, String newPassword) {
    log.debug("Building Steam password reset url");

    String token = fafTokenService.createToken(FafTokenType.PASSWORD_RESET_STEAM,
      Duration.ofHours(6),
      Map.of(
        KEY_STEAM_CALLBACK_URL, callbackUrl,
